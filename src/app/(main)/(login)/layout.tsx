import { media } from "@/public";
import Image from "next/image";
import { text_size } from "../../../utils/constants/css.constants";
import AuthPage from "@/src/components/auth/auth";
export default function LoginLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <div className=" h-[100vh] lg:py-5 p-0 bg-[#222222]">
      <div className="grid grid-cols-12  h-full ">
        <div className="col-span-12 lg:col-span-5">
          <div className="  h-full w-full lg:p-[32px] p-0">
            <div className="w-full h-full lg:rounded-2xl rounded-none overflow-hidden relative">
              <video
                autoPlay
                loop
                muted
                playsInline
                disablePictureInPicture
                controls={false}
                style={{
                  width: "100%",
                  height: "100%",
                  objectFit: "cover",
                }}
              >
                <source src={media.MainBanner} type="video/mp4" />
              </video>
              <div
                className="  absolute inset-0 bg-gradient-to-t from-black via-black/40 to-transparent z-10 flex flex-col 
              justify-end items-center"
              >
                <div className="w-full ">
                  <Image
                    src={media.bannerlogo}
                    alt="Logo"
                    className="flex justify-center items-center"
                    width={900}
                    height={400}
                  />
                </div>
                <p
                  className={`${text_size.p2} text-[#F5E7C6] lg:mb-[66px] mb-[43px]`}
                >
                  Let’s empower your growth through AI
                </p>
              </div>
            </div>
          </div>
        </div>
        <div className="col-span-12 lg:col-span-7 flex justify-center items-center">
          <div className="flex flex-col justify-start items-stretch p-[20px]">
            {children}
          </div>
        </div>
      </div>
    </div>
  );
}
